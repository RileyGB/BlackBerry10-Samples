...
double LocationSession::currentLatitude = double(0.0);
double LocationSession::currentLongitude = double(0.0);
...
void LocationSession::positionUpdated(const QGeoPositionInfo& pos)
{
    ...
    LocationSession::currentLatitude = pos.coordinate().latitude();
    LocationSession::currentLongitude = pos.coordinate().longitude();
...
void LocationSession::positionUpdateTimeout()
{
    ...
    LocationSession::currentLatitude = 0.0;
    LocationSession::currentLongitude = 0.0;
}
...
//        return;
    ...
    /*m_mapView = mapView;
	DataProvider* ddp = new DataProvider();
	GeoLocation* mapPin = new GeoLocation(29.781066, -95.410973, mapView);
	GeoLocation* mapPin2 = new GeoLocation(29.744109, -95.299736, mapView);

	mapPin->setIconUri(QString("asset:///images/icon.png"));
	mapPin2->setIconUri(QString("asset:///images/icon.png"));

	mapPin->setName(QString("Pin 1"));
	mapPin2->setName(QString("Pin 2"));

	ddp->add(mapPin);
	ddp->add(mapPin2);

	MapData* mapData = mapView->mapData();
	mapData->addProvider(ddp);
...
namespace LocationData
{
	double getCurrentLatitude() {
		return LocationSession::currentLatitude;
	}
	double getCurrentLongitude(){
		return LocationSession::currentLongitude;
	}
}
